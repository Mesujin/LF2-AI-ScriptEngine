...
// Global Variables
 ...
 AISCRIPT_NGAMEPTR *neo = (AISCRIPT_NGAMEPTR*)0x458B00;
 //AISCRIPT_NGAME *Ngame = (AISCRIPT_NGAME*)(int32*)(((AISCRIPT_NGAMEPTR*)0x458B00)->game);
 AISCRIPT_NGAME_FILE *Nfile = (AISCRIPT_NGAME_FILE*)0x452980;
 AISCRIPT_INFO self;
...
// Structure/Class Functions Declaration
 ...
 {
  if(object_num < 0 || object_num >= (neora_mode ? _AISCRIPT_PDKNEOMAXOBJNUM : _AISCRIPT_MAXOBJECTNUMBER)) return -1;
  if(neora_mode)
  {if(!neo->game->exists[object_num]) return -1;

   num  = object_num;
   id   = neo->game->objects[object_num]->data->id;
   type = neo->game->objects[object_num]->data->type;

   x = neo->game->objects[object_num]->x;
   y = neo->game->objects[object_num]->y;
   z = neo->game->objects[object_num]->z;
   x_real = neo->game->objects[object_num]->x_real;
   y_real = neo->game->objects[object_num]->y_real;
   z_real = neo->game->objects[object_num]->z_real;
   x_velocity = neo->game->objects[object_num]->x_velocity;
   y_velocity = neo->game->objects[object_num]->y_velocity;
   z_velocity = neo->game->objects[object_num]->z_velocity;

   facing = neo->game->objects[object_num]->facing;
  
   holding_up    = neo->game->objects[object_num]->holding_up;
   holding_left  = neo->game->objects[object_num]->holding_left;
   holding_down  = neo->game->objects[object_num]->holding_down;
   holding_right = neo->game->objects[object_num]->holding_right;
   holding_a     = neo->game->objects[object_num]->holding_a;
   holding_d     = neo->game->objects[object_num]->holding_d;
   holding_j     = neo->game->objects[object_num]->holding_j;
   up    = neo->game->objects[object_num]->up;
   left  = neo->game->objects[object_num]->left;
   down  = neo->game->objects[object_num]->down;
   right = neo->game->objects[object_num]->right;
   A = neo->game->objects[object_num]->A;
   D = neo->game->objects[object_num]->D;
   J = neo->game->objects[object_num]->J;
  
   DrA = neo->game->objects[object_num]->DrA;
   DlA = neo->game->objects[object_num]->DlA;
   DuA = neo->game->objects[object_num]->DuA;
   DdA = neo->game->objects[object_num]->DdA;
   DrJ = neo->game->objects[object_num]->DrJ;
   DlJ = neo->game->objects[object_num]->DlJ;
   DuJ = neo->game->objects[object_num]->DuJ;
   DdJ = neo->game->objects[object_num]->DdJ;
   DJA = neo->game->objects[object_num]->DJA;
  
   hp      = neo->game->objects[object_num]->hp;
   dark_hp = neo->game->objects[object_num]->dark_hp;
   max_hp  = neo->game->objects[object_num]->max_hp;
   mp      = neo->game->objects[object_num]->mp;
  
   frame   = neo->game->objects[object_num]->frame1;
   fall    = neo->game->objects[object_num]->fall;
   bdefend = neo->game->objects[object_num]->bdefend;
   team    = neo->game->objects[object_num]->team;
   blink   = neo->game->objects[object_num]->blink;
   state   = neo->game->objects[object_num]->data->frames[frame].state;

   weapon_type   = neo->game->objects[object_num]->weapon_type;
   weapon_holder = neo->game->objects[object_num]->weapon_holder;
   if(weapon_type == 0){weapon_held = -1;} else {weapon_held = neo->game->objects[object_num]->weapon_held;}

   shake        = neo->game->objects[object_num]->shake;
   wait_counter = neo->game->objects[object_num]->wait_counter;
   ctimer       = neo->game->objects[object_num]->ctimer;
   arest        = neo->game->objects[object_num]->arest;
   memcpy(vrests, neo->game->objects[object_num]->vrests, _AISCRIPT_PDKNEOMAXOBJNUM);

   clone   = neo->game->objects[object_num]->clone;
   reserve = neo->game->objects[object_num]->reserve;
   Ndata    = neo->game->objects[object_num]->data;
  } else
  ...
  int32 vrest_start = (neora_mode ? (*(int32*)neo + 1004) : (0x458c94)) + object_num * 4;
  vrest    = (int32)(*(int8*)(*(int32*)(vrest_start) + 0xF0));
  if(vrest < (int32)(*(int8*)(*(int32*)(vrest_start) + 0xF3))) vrest = (int32)(*(int8*)(*(int32*)(vrest_start) + 0xF3));
...
// Light Functions
 int0   L_Base() perfect
 {
  ...
  int32 Vrab01 = *(int32*)0x44d024;
  if(background != Vrab01)
  {
   background  = Vrab01;
   if(neora_mode)
   {
    if((int32)Nfile > 0 && (int32)Nfile->backgrounds[0] > 0)
    {
     bg_width = Nfile->backgrounds[0]->bg_width;
     bg_zwidth1 = Nfile->backgrounds[0]->bg_zwidth1;
     bg_zwidth2 = Nfile->backgrounds[0]->bg_zwidth2;
	}
   } else
   {
  ...
  current_phase = *(int32*)0x44fb6c;
  current_phase_count = *(int32*)0x44f880;
  current_stage = *(int32*)0x450b94;
  game_is_over = *(int*)0x450b80;
 }
 int0   L_Cleaning(int32 Vrab01)           perfect
 {
  if(Vrab01 < 0 || Vrab01 >= (neora_mode ? _AISCRIPT_PDKNEOMAXOBJNUM : _AISCRIPT_MAXOBJECTNUMBER)) return;
  if(neora_mode)
  {
   if(!neo->game->exists[Vrab01]) return;

   if(neo->game->objects[Vrab01]->up > 0 && key_load[0] > 0) neo->game->objects[Vrab01]->holding_up = 1;
   if(neo->game->objects[Vrab01]->left > 0 && key_load[1] > 0) neo->game->objects[Vrab01]->holding_left = 1;
   if(neo->game->objects[Vrab01]->down > 0 && key_load[2] > 0) neo->game->objects[Vrab01]->holding_down = 1;
   if(neo->game->objects[Vrab01]->right > 0 && key_load[3] > 0) neo->game->objects[Vrab01]->holding_right = 1;
   if(neo->game->objects[Vrab01]->A > 0 && key_load[4] > 0) neo->game->objects[Vrab01]->holding_a = 1;
   if(neo->game->objects[Vrab01]->D > 0 && key_load[5] > 0) neo->game->objects[Vrab01]->holding_d = 1;
   if(neo->game->objects[Vrab01]->J > 0 && key_load[6] > 0) neo->game->objects[Vrab01]->holding_j = 1;
  } else
  {
   if(!game->exists[Vrab01]) return;

   if(game->objects[Vrab01]->up > 0 && key_load[0] > 0) game->objects[Vrab01]->holding_up = 1;
   if(game->objects[Vrab01]->left > 0 && key_load[1] > 0) game->objects[Vrab01]->holding_left = 1;
   if(game->objects[Vrab01]->down > 0 && key_load[2] > 0) game->objects[Vrab01]->holding_down = 1;
   if(game->objects[Vrab01]->right > 0 && key_load[3] > 0) game->objects[Vrab01]->holding_right = 1;
   if(game->objects[Vrab01]->A > 0 && key_load[4] > 0) game->objects[Vrab01]->holding_a = 1;
   if(game->objects[Vrab01]->D > 0 && key_load[5] > 0) game->objects[Vrab01]->holding_d = 1;
   if(game->objects[Vrab01]->J > 0 && key_load[6] > 0) game->objects[Vrab01]->holding_j = 1;
  }
 }
 int0   L_Empty() perfect
 {
  key_lock[0] = false; key_lock[1] = false; key_lock[2] = false; key_lock[3] = false; key_lock[4] = false; key_lock[5] = false; key_lock[6] = false;
  key_protect[0] = false; key_protect[1] = false; key_protect[2] = false; key_protect[3] = false; key_protect[4] = false; key_protect[5] = false; key_protect[6] = false;
 }
 int0   L_Protect(statics int32 Vrab01, statics int32 Vrab02, statics int32 Vrab03, statics int32 Vrab04) perfect
 {
  if(neora_mode)
  {
  for(int32 Vrab05 = 0; Vrab05 < 4; ++Vrab05) switch(Vrab05 == 0 ? Vrab01 : (Vrab05 == 1 ? Vrab02 : (Vrab05 == 2 ? Vrab03 : Vrab04)))
   {
    case 0:
     key_protect[0] = true;
     if(neo->game->objects[self.num]->holding_up == 0){neo->game->objects[self.num]->up = 0; self.up = 0;}
    break;
    case 1:
     key_protect[1] = true;
     if(neo->game->objects[self.num]->holding_left == 0){neo->game->objects[self.num]->left = 0; self.left = 0;}
    break;
    case 2:
     key_protect[2] = true;
     if(neo->game->objects[self.num]->holding_down == 0){neo->game->objects[self.num]->down = 0; self.down = 0;}
    break;
    case 3:
     key_protect[3] = true;
     if(neo->game->objects[self.num]->holding_right == 0){neo->game->objects[self.num]->right = 0; self.right = 0;}
    break;
    case 4:
     key_protect[4] = true;
     if(neo->game->objects[self.num]->holding_a == 0){neo->game->objects[self.num]->A = 0; self.A = 0;}
    break;
    case 5:
     key_protect[5] = true;
     if(neo->game->objects[self.num]->holding_d == 0){neo->game->objects[self.num]->D = 0; self.D = 0;}
    break;
    case 6:
     key_protect[6] = true;
     if(neo->game->objects[self.num]->holding_j == 0){neo->game->objects[self.num]->J = 0; self.J = 0;}
    break;
    default: break;
   }
  } else
  {
   for(int32 Vrab05 = 0; Vrab05 < 4; ++Vrab05) switch(Vrab05 == 0 ? Vrab01 : (Vrab05 == 1 ? Vrab02 : (Vrab05 == 2 ? Vrab03 : Vrab04)))
   {
    case 0:
     key_protect[0] = true;
     if(game->objects[self.num]->holding_up == 0){game->objects[self.num]->up = 0; self.up = 0;}
    break;
    case 1:
     key_protect[1] = true;
     if(game->objects[self.num]->holding_left == 0){game->objects[self.num]->left = 0; self.left = 0;}
    break;
    case 2:
     key_protect[2] = true;
     if(game->objects[self.num]->holding_down == 0){game->objects[self.num]->down = 0; self.down = 0;}
    break;
    case 3:
     key_protect[3] = true;
     if(game->objects[self.num]->holding_right == 0){game->objects[self.num]->right = 0; self.right = 0;}
    break;
    case 4:
     key_protect[4] = true;
     if(game->objects[self.num]->holding_a == 0){game->objects[self.num]->A = 0; self.A = 0;}
    break;
    case 5:
     key_protect[5] = true;
     if(game->objects[self.num]->holding_d == 0){game->objects[self.num]->D = 0; self.D = 0;}
    break;
    case 6:
     key_protect[6] = true;
     if(game->objects[self.num]->holding_j == 0){game->objects[self.num]->J = 0; self.J = 0;}
    break;
    default: break;
   }
  }
 }
 int0   L_Lock(statics int32 Vrab01)   perfect 
 {
  if(neora_mode)
  {
   switch(Vrab01)
   {
    case 0:
     key_lock[0] = true;
     neo->game->objects[self.num]->up = 0; self.up = 0;
     neo->game->objects[self.num]->holding_up = 0; self.holding_up = 0;
    break;
    case 1:
     key_lock[1] = true;
     neo->game->objects[self.num]->left = 0; self.left = 0;
     neo->game->objects[self.num]->holding_left = 0; self.holding_left = 0;
    break;
    case 2:
     key_lock[2] = true;
     neo->game->objects[self.num]->down = 0; self.down = 0;
     neo->game->objects[self.num]->holding_down = 0; self.holding_down = 0;
    break;
    case 3:
     key_lock[3] = true;
     neo->game->objects[self.num]->right = 0; self.right = 0;
     neo->game->objects[self.num]->holding_right = 0; self.holding_right = 0;
    break;
    case 4:
     key_lock[4] = true;
     neo->game->objects[self.num]->A = 0; self.A = 0;
     neo->game->objects[self.num]->holding_a = 0; self.holding_a = 0;
    break;
    case 5:
     key_lock[5] = true;
     neo->game->objects[self.num]->D = 0; self.D = 0;
     neo->game->objects[self.num]->holding_d = 0; self.holding_d = 0;
    break;
    case 6:
     key_lock[6] = true;
     neo->game->objects[self.num]->J = 0; self.J = 0;
     neo->game->objects[self.num]->holding_j = 0; self.holding_j = 0;
    break;
    default: break;
   }
  } else
 ...
 int1   L_Control(int32 Vrab01) perfect
 {
  if(Vrab01 < 0 || Vrab01 >= (neora_mode ? _AISCRIPT_PDKNEOMAXOBJNUM : _AISCRIPT_MAXOBJECTNUMBER)) return false;
  if(neora_mode)
  {if(!neo->game->exists[Vrab01]) return false;} else {if(!game->exists[Vrab01]) return false;}

  key_lock[0] = false; key_lock[1] = false; key_lock[2] = false; key_lock[3] = false; key_lock[4] = false; key_lock[5] = false; key_lock[6] = false;
  key_protect[0] = false; key_protect[1] = false; key_protect[2] = false; key_protect[3] = false; key_protect[4] = false; key_protect[5] = false; key_protect[6] = false;
  key_load[0] = 0; key_load[1] = 0; key_load[2] = 0; key_load[3] = 0; key_load[4] = 0; key_load[5] = 0; key_load[6] = 0;
  
  if(neora_mode)
  {
   if(neo->game->objects[Vrab01]->holding_up != 0){key_load[0] = 2;} else
   {if(neo->game->objects[Vrab01]->up != 0){key_load[0] = 1; neo->game->objects[Vrab01]->up = 1; neo->game->objects[Vrab01]->holding_up = 1;}}
   if(neo->game->objects[Vrab01]->holding_left != 0){key_load[1] = 2;} else
   {if(neo->game->objects[Vrab01]->left != 0){key_load[1] = 1; neo->game->objects[Vrab01]->left = 1; neo->game->objects[Vrab01]->holding_left = 1;}}
   if(neo->game->objects[Vrab01]->holding_down != 0){key_load[2] = 2;} else
   {if(neo->game->objects[Vrab01]->down != 0){key_load[2] = 1; neo->game->objects[Vrab01]->down = 1; neo->game->objects[Vrab01]->holding_down = 1;}}
   if(neo->game->objects[Vrab01]->holding_right != 0){key_load[3] = 2;} else
   {if(neo->game->objects[Vrab01]->right != 0){key_load[3] = 1; neo->game->objects[Vrab01]->right = 1; neo->game->objects[Vrab01]->holding_right = 1;}}
   if(neo->game->objects[Vrab01]->holding_a != 0){key_load[4] = 2;} else
   {if(neo->game->objects[Vrab01]->A != 0){key_load[4] = 1; neo->game->objects[Vrab01]->A = 1; neo->game->objects[Vrab01]->holding_a = 1;}}
   if(neo->game->objects[Vrab01]->holding_d != 0){key_load[5] = 2;} else
   {if(neo->game->objects[Vrab01]->D != 0){key_load[5] = 1; neo->game->objects[Vrab01]->D = 1; neo->game->objects[Vrab01]->holding_d = 1;}}
   if(neo->game->objects[Vrab01]->holding_j != 0){key_load[6] = 2;} else
   {if(neo->game->objects[Vrab01]->J != 0){key_load[6] = 1; neo->game->objects[Vrab01]->J = 1; neo->game->objects[Vrab01]->holding_j = 1;}}
  } else
  {
   if(game->objects[Vrab01]->holding_up != 0){key_load[0] = 2;} else
   {if(game->objects[Vrab01]->up != 0){key_load[0] = 1; game->objects[Vrab01]->up = 1; game->objects[Vrab01]->holding_up = 1;}}
   if(game->objects[Vrab01]->holding_left != 0){key_load[1] = 2;} else
   {if(game->objects[Vrab01]->left != 0){key_load[1] = 1; game->objects[Vrab01]->left = 1; game->objects[Vrab01]->holding_left = 1;}}
   if(game->objects[Vrab01]->holding_down != 0){key_load[2] = 2;} else
   {if(game->objects[Vrab01]->down != 0){key_load[2] = 1; game->objects[Vrab01]->down = 1; game->objects[Vrab01]->holding_down = 1;}}
   if(game->objects[Vrab01]->holding_right != 0){key_load[3] = 2;} else
   {if(game->objects[Vrab01]->right != 0){key_load[3] = 1; game->objects[Vrab01]->right = 1; game->objects[Vrab01]->holding_right = 1;}}
   if(game->objects[Vrab01]->holding_a != 0){key_load[4] = 2;} else
   {if(game->objects[Vrab01]->A != 0){key_load[4] = 1; game->objects[Vrab01]->A = 1; game->objects[Vrab01]->holding_a = 1;}}
   if(game->objects[Vrab01]->holding_d != 0){key_load[5] = 2;} else
   {if(game->objects[Vrab01]->D != 0){key_load[5] = 1; game->objects[Vrab01]->D = 1; game->objects[Vrab01]->holding_d = 1;}}
   if(game->objects[Vrab01]->holding_j != 0){key_load[6] = 2;} else
   {if(game->objects[Vrab01]->J != 0){key_load[6] = 1; game->objects[Vrab01]->J = 1; game->objects[Vrab01]->holding_j = 1;}}
  }
  return true;
 }
...
// Shared Functions
 ...
 int32 setEnemy(int32 object_num){if(object_num < 0 || object_num >= (neora_mode ? _AISCRIPT_PDKNEOMAXOBJNUM : _AISCRIPT_MAXOBJECTNUMBER)) return -1; if(neora_mode){if(!neo->game->exists[object_num]) return -1; return neo->game->objects[self.num]->enemy = object_num;} else {if(!game->exists[object_num]) return -1; return game->objects[self.num]->enemy = object_num;}}
 
 int0  up(int8 key, int8 holding)
 {
  if(key_lock[0]) return;
  ...
  if(key_protect[0]) if(Vrab01 && !Vrab02) Vrab01 = false;
  
  if(neora_mode)
  {
   neo->game->objects[self.num]->up = Vrab01 ? 1 : 0;
   neo->game->objects[self.num]->holding_up = Vrab02 ? 1 : 0;
  } else
  {
 ...
 int0  left(int8 key, int8 holding)
 {
  ...
  if(key_protect[1]) if(Vrab01 && !Vrab02) Vrab01 = false;
  
  if(neora_mode)
  {
   neo->game->objects[self.num]->left = Vrab01 ? 1 : 0;
   neo->game->objects[self.num]->holding_left = Vrab02 ? 1 : 0;
  } else
  {
 ...
 int0  down(int8 key, int8 holding)
 {
  ...
  if(key_protect[2]) if(Vrab01 && !Vrab02) Vrab01 = false;
  
  if(neora_mode)
  {
   neo->game->objects[self.num]->down = Vrab01 ? 1 : 0;
   neo->game->objects[self.num]->holding_down = Vrab02 ? 1 : 0;
  } else
  {
 ...
 int0  right(int8 key, int8 holding)
 {
  ...
  if(key_protect[3]) if(Vrab01 && !Vrab02) Vrab01 = false;
  
  if(neora_mode)
  {
   neo->game->objects[self.num]->right = Vrab01 ? 1 : 0;
   neo->game->objects[self.num]->holding_right = Vrab02 ? 1 : 0;
  } else
  {
 ...
 int0  A(int8 key, int8 holding)
 {
  ...
  if(key_protect[4]) if(Vrab01 && !Vrab02) Vrab01 = false;
  
  if(neora_mode)
  {
   neo->game->objects[self.num]->A = Vrab01 ? 1 : 0;
   neo->game->objects[self.num]->holding_a = Vrab02 ? 1 : 0;
  } else
  {
 ...
 int0  D(int8 key, int8 holding)
 {
  ...
  if(key_protect[5]) if(Vrab01 && !Vrab02) Vrab01 = false;
  
  if(neora_mode)
  {
   neo->game->objects[self.num]->D = Vrab01 ? 1 : 0;
   neo->game->objects[self.num]->holding_d = Vrab02 ? 1 : 0;
  } else
  {
 ...
 int0  J(int8 key, int8 holding)
 {
  ...
  if(key_protect[6]) if(Vrab01 && !Vrab02) Vrab01 = false;
  
  if(neora_mode)
  {
   neo->game->objects[self.num]->J = Vrab01 ? 1 : 0;
   neo->game->objects[self.num]->holding_j = Vrab02 ? 1 : 0;
  } else
  {
 ...
 int0  DrA() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DrA;} else {Vrab01 = game->objects[self.num]->DrA;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[3] == 0 && key_load[4] == 0)
  {L_Protect(0, 1, 2, 6); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[3] == 0 && key_load[4] == 0)
  {L_Protect(0, 1, 2, 6); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[4] == 0)
  {L_Protect(0, 1, 2, 6); Vrab04 = true;}
 
  L_Lock(5); L_Lock(3); L_Lock(4);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D     = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->right = Vrab03 ? 1 : 0;
   neo->game->objects[self.num]->A     = Vrab04 ? 1 : 0;
  } else
  {
   game->objects[self.num]->D     = Vrab02 ? 1 : 0; 
   game->objects[self.num]->right = Vrab03 ? 1 : 0;
   game->objects[self.num]->A     = Vrab04 ? 1 : 0;
  }
  self.D = Vrab02 ? 1 : 0; self.right = Vrab03 ? 1 : 0; self.A = Vrab04 ? 1 : 0;
 }
 int0  DrJ() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DrJ;} else {Vrab01 = game->objects[self.num]->DrJ;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[3] == 0 && key_load[6] == 0)
  {L_Protect(0, 1, 2, 4); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[3] == 0 && key_load[6] == 0)
  {L_Protect(0, 1, 2, 4); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[6] == 0)
  {L_Protect(0, 1, 2, 4); Vrab04 = true;}
 
  L_Lock(5); L_Lock(3); L_Lock(6);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D     = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->right = Vrab03 ? 1 : 0;
   neo->game->objects[self.num]->J     = Vrab04 ? 1 : 0;
  } else
  {
   game->objects[self.num]->D     = Vrab02 ? 1 : 0; 
   game->objects[self.num]->right = Vrab03 ? 1 : 0;
   game->objects[self.num]->J     = Vrab04 ? 1 : 0;
  }
  self.D = Vrab02 ? 1 : 0; self.right = Vrab03 ? 1 : 0; self.J = Vrab04 ? 1 : 0;
 }
 int0  DlA() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DlA;} else {Vrab01 = game->objects[self.num]->DlA;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[1] == 0 && key_load[4] == 0)
  {L_Protect(0, 2, 3, 6); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[1] == 0 && key_load[4] == 0)
  {L_Protect(0, 2, 3, 6); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[4] == 0)
  {L_Protect(0, 2, 3, 6); Vrab04 = true;}
 
  L_Lock(5); L_Lock(1); L_Lock(4);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D    = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->left = Vrab03 ? 1 : 0;
   neo->game->objects[self.num]->A    = Vrab04 ? 1 : 0;
  } else
  {
   game->objects[self.num]->D     = Vrab02 ? 1 : 0; 
   game->objects[self.num]->left = Vrab03 ? 1 : 0;
   game->objects[self.num]->A     = Vrab04 ? 1 : 0;
  }
  self.D = Vrab02 ? 1 : 0; self.left = Vrab03 ? 1 : 0; self.A = Vrab04 ? 1 : 0;
 }
 int0  DlJ() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DlJ;} else {Vrab01 = game->objects[self.num]->DlJ;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[1] == 0 && key_load[6] == 0)
  {L_Protect(0, 2, 3, 4); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[1] == 0 && key_load[6] == 0)
  {L_Protect(0, 2, 3, 4); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[6] == 0)
  {L_Protect(0, 2, 3, 4); Vrab04 = true;}
 
  L_Lock(5); L_Lock(1); L_Lock(6);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D    = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->left = Vrab03 ? 1 : 0;
  ...
  self.D = Vrab02 ? 1 : 0; self.left = Vrab03 ? 1 : 0; self.J = Vrab04 ? 1 : 0;
 }
 int0  DdA() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DdA;} else {Vrab01 = game->objects[self.num]->DdA;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[2] == 0 && key_load[4] == 0)
  {L_Protect(0, 1, 3, 6); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[2] == 0 && key_load[4] == 0)
  {L_Protect(0, 1, 3, 6); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[4] == 0)
  {L_Protect(0, 1, 3, 6); Vrab04 = true;}
 
  L_Lock(5); L_Lock(2); L_Lock(4);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D    = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->down = Vrab03 ? 1 : 0;
   neo->game->objects[self.num]->A    = Vrab04 ? 1 : 0;
  } else
  {
   game->objects[self.num]->D     = Vrab02 ? 1 : 0; 
   game->objects[self.num]->down = Vrab03 ? 1 : 0;
   game->objects[self.num]->A     = Vrab04 ? 1 : 0;
  }
  self.D = Vrab02 ? 1 : 0; self.down = Vrab03 ? 1 : 0; self.A = Vrab04 ? 1 : 0;
 }
 int0  DdJ() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DdJ;} else {Vrab01 = game->objects[self.num]->DdJ;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[2] == 0 && key_load[6] == 0)
  {L_Protect(0, 1, 3, 4); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[2] == 0 && key_load[6] == 0)
  {L_Protect(0, 1, 3, 4); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[6] == 0)
  {L_Protect(0, 1, 3, 4); Vrab04 = true;}
 
  L_Lock(5); L_Lock(2); L_Lock(6);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D    = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->down = Vrab03 ? 1 : 0;
  ...
  self.D = Vrab02 ? 1 : 0; self.down = Vrab03 ? 1 : 0; self.J = Vrab04 ? 1 : 0;
 }
 int0  DuA() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DuA;} else {Vrab01 = game->objects[self.num]->DuA;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[0] == 0 && key_load[4] == 0)
  {L_Protect(1, 2, 3, 6); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[0] == 0 && key_load[4] == 0)
  {L_Protect(1, 2, 3, 6); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[4] == 0)
  {L_Protect(1, 2, 3, 6); Vrab04 = true;}
 
  L_Lock(5); L_Lock(0); L_Lock(4);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D  = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->up = Vrab03 ? 1 : 0;
   neo->game->objects[self.num]->A  = Vrab04 ? 1 : 0;
  } else
  {
   game->objects[self.num]->D  = Vrab02 ? 1 : 0; 
   game->objects[self.num]->up = Vrab03 ? 1 : 0;
   game->objects[self.num]->A  = Vrab04 ? 1 : 0;
  }
  self.D = Vrab02 ? 1 : 0; self.up = Vrab03 ? 1 : 0; self.A = Vrab04 ? 1 : 0;
 }
 int0  DuJ() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DuJ;} else {Vrab01 = game->objects[self.num]->DuJ;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[0] == 0 && key_load[6] == 0)
  {L_Protect(1, 2, 3, 4); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[0] == 0 && key_load[6] == 0)
  {L_Protect(1, 2, 3, 4); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[6] == 0)
  {L_Protect(1, 2, 3, 4); Vrab04 = true;}
 
  L_Lock(5); L_Lock(0); L_Lock(6);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D  = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->up = Vrab03 ? 1 : 0;
  ...
  self.D = Vrab02 ? 1 : 0; self.up = Vrab03 ? 1 : 0; self.J = Vrab04 ? 1 : 0;
 }
 int0  DJA() perfect
 {
  int8 Vrab01 = 0; if(neora_mode){Vrab01 = neo->game->objects[self.num]->DJA;} else {Vrab01 = game->objects[self.num]->DJA;}
  int1 Vrab02 = false, Vrab03 = false, Vrab04 = false; L_Empty();

  if(Vrab01 == 0)
  if(key_load[5] == 0 && key_load[6] == 0 && key_load[4] == 0)
  {L_Protect(0, 1, 2, 3); Vrab02 = true; Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 1)
  if(key_load[6] == 0 && key_load[4] == 0)
  {L_Protect(0, 1, 2, 3); Vrab03 = true; Vrab04 = true;}
  if(Vrab01 == 2)
  if(key_load[4] == 0)
  {L_Protect(0, 1, 2, 3); Vrab04 = true;}
 
  L_Lock(5); L_Lock(6); L_Lock(4);
  if(neora_mode)
  {
   neo->game->objects[self.num]->D = Vrab02 ? 1 : 0; 
   neo->game->objects[self.num]->J = Vrab03 ? 1 : 0;
   neo->game->objects[self.num]->A = Vrab04 ? 1 : 0;
  } else
  {
   game->objects[self.num]->D = Vrab02 ? 1 : 0; 
   game->objects[self.num]->J = Vrab03 ? 1 : 0;
   game->objects[self.num]->A = Vrab04 ? 1 : 0;
  }
  self.D = Vrab02 ? 1 : 0; self.J = Vrab03 ? 1 : 0; self.A = Vrab04 ? 1 : 0;
 }
...
// Detour Functions
 ...
 {
  string Temp01 = std::to_string(0) + ".as";
 ...
 int0  __stdcall D_ID(int32 object_num, int32 unkwn1)
 {
  ...
  if(elapsed_time != *(int32*)0x450B8C || neora_mode) L_Base();
  if(!L_Control(object_num)) return;

  #if _AISCRIPT_SLOWINPUTUPDATE
   if(elapsed_time % 2 == 1) return;
  #endif

  int32 id_int = -1;
  if(neora_mode){id_int = neo->game->objects[object_num]->data->id;} else {id_int = game->objects[object_num]->data->id;}
  ...
  if(full_script)
  {
   ...
   if(ScriptModule)
   {
    ...
    if(Function){D_CallID(object_num, Function); Vrab01 = true;} else
    {
     ...
     if(Function){D_CallID(object_num, Function); statics int32 Vrab02 = ScriptContext->GetReturnDWord(); if(Vrab02 == 0) return; Vrab01 = (Vrab02 == 1);}
     #ifdef AISCRIPT_DEBUG
      else
      {
 ...
 int32 __stdcall D_EGO(int32 target_num, int32 object_num, int32 unkwn1, int32 unkwn2, int32 unkwn3, int32 unkwn4, int32 unkwn5)
 {
  ...
  L_Cleaning(object_num);

  if(neora_mode)
  {ScriptModule = ScriptEngine->GetModule(std::to_string(neo->game->objects[object_num]->data->id).c_str());} else
...
// Main Functions
 ...
 int0 M_Functions() perfect
 {
  ...
  if(neora_mode)
  {
   ScriptEngine->RegisterObjectProperty("Game", "const int32 state",         asOFFSET(AISCRIPT_NGAME, state));
   ScriptEngine->RegisterObjectProperty("Game", "const BoolArray exists",    asOFFSET(AISCRIPT_NGAME, exists));
   ScriptEngine->RegisterObjectProperty("Game", "const ObjectArray objects", asOFFSET(AISCRIPT_NGAME, objects));
   //ScriptEngine->RegisterObjectProperty("Game", "const FileManager @files",  0, (int32)Nfile, true);
  
   ScriptEngine->RegisterObjectProperty("FileManager", "const DataFileArray datas", asOFFSET(AISCRIPT_NGAME_FILE, datas));
   ScriptEngine->RegisterObjectProperty("FileManager", "const IntArray data_ptrs",  asOFFSET(AISCRIPT_NGAME_FILE, datas)); //allows to check whether or not a datafile exists
   ScriptEngine->RegisterObjectProperty("FileManager", "const StageArray stages",   asOFFSET(AISCRIPT_NGAME_FILE, stages));
   ScriptEngine->RegisterObjectMethod  ("FileManager", "const BackgroundArray &get_backgrounds() const", asFUNCTION(Index < asOFFSET(AISCRIPT_NGAME_FILE, backgrounds) >), asCALL_CDECL_OBJLAST);

   ScriptEngine->RegisterObjectProperty("Object", "const int32  move_counter", asOFFSET(AISCRIPT_NGAME_OBJECT, move_counter));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  run_counter",  asOFFSET(AISCRIPT_NGAME_OBJECT, run_counter));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  blink",        asOFFSET(AISCRIPT_NGAME_OBJECT, blink));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  unkwn1",       asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn1));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  x", asOFFSET(AISCRIPT_NGAME_OBJECT, x));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  y", asOFFSET(AISCRIPT_NGAME_OBJECT, y));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  z", asOFFSET(AISCRIPT_NGAME_OBJECT, z));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn2",      asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn2));
   ScriptEngine->RegisterObjectProperty("Object", "const double x_acceleration", asOFFSET(AISCRIPT_NGAME_OBJECT, x_acceleration));
   ScriptEngine->RegisterObjectProperty("Object", "const double y_acceleration", asOFFSET(AISCRIPT_NGAME_OBJECT, y_acceleration));
   ScriptEngine->RegisterObjectProperty("Object", "const double z_acceleration", asOFFSET(AISCRIPT_NGAME_OBJECT, z_acceleration));
   ScriptEngine->RegisterObjectProperty("Object", "const double x_velocity", asOFFSET(AISCRIPT_NGAME_OBJECT, x_velocity));
   ScriptEngine->RegisterObjectProperty("Object", "const double y_velocity", asOFFSET(AISCRIPT_NGAME_OBJECT, y_velocity));
   ScriptEngine->RegisterObjectProperty("Object", "const double z_velocity", asOFFSET(AISCRIPT_NGAME_OBJECT, z_velocity));
   ScriptEngine->RegisterObjectProperty("Object", "const double x_real", asOFFSET(AISCRIPT_NGAME_OBJECT, x_real));
   ScriptEngine->RegisterObjectProperty("Object", "const double y_real", asOFFSET(AISCRIPT_NGAME_OBJECT, y_real));
   ScriptEngine->RegisterObjectProperty("Object", "const double z_real", asOFFSET(AISCRIPT_NGAME_OBJECT, z_real));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  frame1", asOFFSET(AISCRIPT_NGAME_OBJECT, frame1));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  frame2", asOFFSET(AISCRIPT_NGAME_OBJECT, frame2));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  frame3", asOFFSET(AISCRIPT_NGAME_OBJECT, frame3));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  frame4", asOFFSET(AISCRIPT_NGAME_OBJECT, frame4));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   facing", asOFFSET(AISCRIPT_NGAME_OBJECT, facing));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn3",     asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn3));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  wait_counter",  asOFFSET(AISCRIPT_NGAME_OBJECT, wait_counter));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  ccatching",     asOFFSET(AISCRIPT_NGAME_OBJECT, ccatching));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  ccatcher",      asOFFSET(AISCRIPT_NGAME_OBJECT, ccatcher));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  ctimer",        asOFFSET(AISCRIPT_NGAME_OBJECT, ctimer));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  weapon_type",   asOFFSET(AISCRIPT_NGAME_OBJECT, weapon_type));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  weapon_held",   asOFFSET(AISCRIPT_NGAME_OBJECT, weapon_held));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  weapon_holder", asOFFSET(AISCRIPT_NGAME_OBJECT, weapon_holder));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  unkwn4",        asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn4));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn5",     asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn5));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  fall",          asOFFSET(AISCRIPT_NGAME_OBJECT, fall));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  shake",         asOFFSET(AISCRIPT_NGAME_OBJECT, shake));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  bdefend",       asOFFSET(AISCRIPT_NGAME_OBJECT, bdefend));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn6",     asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn6));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   holding_up",    asOFFSET(AISCRIPT_NGAME_OBJECT, holding_up));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   holding_down",  asOFFSET(AISCRIPT_NGAME_OBJECT, holding_down));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   holding_left",  asOFFSET(AISCRIPT_NGAME_OBJECT, holding_left));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   holding_right", asOFFSET(AISCRIPT_NGAME_OBJECT, holding_right));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   holding_a",     asOFFSET(AISCRIPT_NGAME_OBJECT, holding_a));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   holding_j",     asOFFSET(AISCRIPT_NGAME_OBJECT, holding_j));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   holding_d",     asOFFSET(AISCRIPT_NGAME_OBJECT, holding_d));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   up",    asOFFSET(AISCRIPT_NGAME_OBJECT, up));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   down",  asOFFSET(AISCRIPT_NGAME_OBJECT, down));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   left",  asOFFSET(AISCRIPT_NGAME_OBJECT, left));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   right", asOFFSET(AISCRIPT_NGAME_OBJECT, right));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   A",     asOFFSET(AISCRIPT_NGAME_OBJECT, A));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   J",     asOFFSET(AISCRIPT_NGAME_OBJECT, J));
   ScriptEngine->RegisterObjectProperty("Object", "const bool   D",     asOFFSET(AISCRIPT_NGAME_OBJECT, D));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DrA", asOFFSET(AISCRIPT_NGAME_OBJECT, DrA));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DlA", asOFFSET(AISCRIPT_NGAME_OBJECT, DlA));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DuA", asOFFSET(AISCRIPT_NGAME_OBJECT, DuA));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DdA", asOFFSET(AISCRIPT_NGAME_OBJECT, DdA));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DrJ", asOFFSET(AISCRIPT_NGAME_OBJECT, DrJ));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DlJ", asOFFSET(AISCRIPT_NGAME_OBJECT, DlJ));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DuJ", asOFFSET(AISCRIPT_NGAME_OBJECT, DuJ));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DdJ", asOFFSET(AISCRIPT_NGAME_OBJECT, DdJ));
   ScriptEngine->RegisterObjectProperty("Object", "const int8   DJA", asOFFSET(AISCRIPT_NGAME_OBJECT, DJA));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn7", asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn7));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  heal",      asOFFSET(AISCRIPT_NGAME_OBJECT, heal));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn8", asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn8));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  arest",     asOFFSET(AISCRIPT_NGAME_OBJECT, arest));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray vrests", asOFFSET(AISCRIPT_NGAME_OBJECT, vrests));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  vrest",     asOFFSET(AISCRIPT_NGAME_OBJECT, vrests));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  attacked_object_num", asOFFSET(AISCRIPT_NGAME_OBJECT, attacked_object_num));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn9",           asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn9));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  clone",               asOFFSET(AISCRIPT_NGAME_OBJECT, clone));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  weapon_thrower",      asOFFSET(AISCRIPT_NGAME_OBJECT, weapon_thrower));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  hp",      asOFFSET(AISCRIPT_NGAME_OBJECT, hp));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  dark_hp", asOFFSET(AISCRIPT_NGAME_OBJECT, dark_hp));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  max_hp",  asOFFSET(AISCRIPT_NGAME_OBJECT, max_hp));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  mp",      asOFFSET(AISCRIPT_NGAME_OBJECT, mp));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  reserve", asOFFSET(AISCRIPT_NGAME_OBJECT, reserve));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  unkwn10", asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn10));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  unkwn11", asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn11));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  pic_gain",          asOFFSET(AISCRIPT_NGAME_OBJECT, pic_gain));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  bottle_hp",         asOFFSET(AISCRIPT_NGAME_OBJECT, bottle_hp));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  throwinjury",       asOFFSET(AISCRIPT_NGAME_OBJECT, throwinjury));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn12",        asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn12));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  firzen_counter",    asOFFSET(AISCRIPT_NGAME_OBJECT, firzen_counter));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  unkwn13",           asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn13));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  armour_multiplier", asOFFSET(AISCRIPT_NGAME_OBJECT, armour_multiplier));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  unkwn14",           asOFFSET(AISCRIPT_NGAME_OBJECT, unkwn14));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  total_attack",      asOFFSET(AISCRIPT_NGAME_OBJECT, total_attack));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  hp_lost",           asOFFSET(AISCRIPT_NGAME_OBJECT, hp_lost));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  mp_usage",          asOFFSET(AISCRIPT_NGAME_OBJECT, mp_usage));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  host",              asOFFSET(AISCRIPT_NGAME_OBJECT, host));
   ScriptEngine->RegisterObjectProperty("Object", "const CharArray unkwn15",        asOFFSET(AISCRIPT_NGAME_OBJECT, host));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  kills",             asOFFSET(AISCRIPT_NGAME_OBJECT, kills));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  weapon_picks",      asOFFSET(AISCRIPT_NGAME_OBJECT, weapon_picks));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  enemy",             asOFFSET(AISCRIPT_NGAME_OBJECT, enemy));
   ScriptEngine->RegisterObjectProperty("Object", "const int32  team",              asOFFSET(AISCRIPT_NGAME_OBJECT, team));
   ScriptEngine->RegisterObjectProperty("Object", "const DataFile @data",           asOFFSET(AISCRIPT_NGAME_OBJECT, data));
  
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  walking_frame_rate", asOFFSET(AISCRIPT_NGAME_FILE_DATA, walking_frame_rate));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  unkwn1",             asOFFSET(AISCRIPT_NGAME_FILE_DATA, unkwn1));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double walking_speed",      asOFFSET(AISCRIPT_NGAME_FILE_DATA, walking_speed));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double walking_speedz",     asOFFSET(AISCRIPT_NGAME_FILE_DATA, walking_speedz));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  running_frame_rate", asOFFSET(AISCRIPT_NGAME_FILE_DATA, running_frame_rate));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double running_speed",      asOFFSET(AISCRIPT_NGAME_FILE_DATA, running_speed));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double running_speedz",     asOFFSET(AISCRIPT_NGAME_FILE_DATA, running_speedz));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double heavy_walking_speed",  asOFFSET(AISCRIPT_NGAME_FILE_DATA, heavy_walking_speed));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double heavy_walking_speedz", asOFFSET(AISCRIPT_NGAME_FILE_DATA, heavy_walking_speedz));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double heavy_running_speed",  asOFFSET(AISCRIPT_NGAME_FILE_DATA, heavy_running_speed));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double heavy_running_speedz", asOFFSET(AISCRIPT_NGAME_FILE_DATA, heavy_running_speedz));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double jump_height",      asOFFSET(AISCRIPT_NGAME_FILE_DATA, jump_height));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double jump_distance",    asOFFSET(AISCRIPT_NGAME_FILE_DATA, jump_distance));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double jump_distancez",   asOFFSET(AISCRIPT_NGAME_FILE_DATA, jump_distancez));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double dash_height",      asOFFSET(AISCRIPT_NGAME_FILE_DATA, dash_height));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double dash_distance",    asOFFSET(AISCRIPT_NGAME_FILE_DATA, dash_distance));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double dash_distancez",   asOFFSET(AISCRIPT_NGAME_FILE_DATA, dash_distancez));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double rowing_height",    asOFFSET(AISCRIPT_NGAME_FILE_DATA, rowing_height));
   ScriptEngine->RegisterObjectProperty("DataFile", "const double rowing_distance",  asOFFSET(AISCRIPT_NGAME_FILE_DATA, rowing_distance));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  weapon_hp",        asOFFSET(AISCRIPT_NGAME_FILE_DATA, weapon_hp));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  weapon_drop_hurt", asOFFSET(AISCRIPT_NGAME_FILE_DATA, weapon_drop_hurt));
   ScriptEngine->RegisterObjectProperty("DataFile", "const CharArray unkwn2",        asOFFSET(AISCRIPT_NGAME_FILE_DATA, unkwn2));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  file_count",       asOFFSET(AISCRIPT_NGAME_FILE_DATA, file_count));
   ScriptEngine->RegisterObjectProperty("DataFile", "const CharArrayArray40 file_address", asOFFSET(AISCRIPT_NGAME_FILE_DATA, file_address));
   ScriptEngine->RegisterObjectProperty("DataFile", "const FileArray file", asOFFSET(AISCRIPT_NGAME_FILE_DATA, file));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  id",      asOFFSET(AISCRIPT_NGAME_FILE_DATA, id));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  type",    asOFFSET(AISCRIPT_NGAME_FILE_DATA, type));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  unkwn3",  asOFFSET(AISCRIPT_NGAME_FILE_DATA, unkwn3));
   ScriptEngine->RegisterObjectProperty("DataFile", "const CharArray small_bmp", asOFFSET(AISCRIPT_NGAME_FILE_DATA, small_bmp));
   ScriptEngine->RegisterObjectProperty("DataFile", "const int32  unkwn4",       asOFFSET(AISCRIPT_NGAME_FILE_DATA, unkwn4));
   ScriptEngine->RegisterObjectProperty("DataFile", "const CharArray  face_bmp", asOFFSET(AISCRIPT_NGAME_FILE_DATA, face_bmp));
   ScriptEngine->RegisterObjectProperty("DataFile", "const IntArray   unkwn5",   asOFFSET(AISCRIPT_NGAME_FILE_DATA, unkwn4));
   ScriptEngine->RegisterObjectProperty("DataFile", "const FrameArray frames",   asOFFSET(AISCRIPT_NGAME_FILE_DATA, frames));
   ScriptEngine->RegisterObjectMethod  ("DataFile", "const CharArray  &get_name() const", asFUNCTION(Index < asOFFSET(AISCRIPT_NGAME_FILE_DATA, name) >), asCALL_CDECL_OBJLAST);
    
   ScriptEngine->RegisterObjectProperty("File", "const int32 ptr", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FILE, ptr[0]));
   ScriptEngine->RegisterObjectProperty("File", "const int32 w",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FILE, w[0]));
   ScriptEngine->RegisterObjectProperty("File", "const int32 h",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FILE, h[0]));
   ScriptEngine->RegisterObjectProperty("File", "const int32 row", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FILE, row[0]));
   ScriptEngine->RegisterObjectProperty("File", "const int32 col", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FILE, col[0]));

   ScriptEngine->RegisterObjectProperty("Strength", "const EntryArray entry",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH, entry[0]));
   ScriptEngine->RegisterObjectProperty("Strength", "const CharArrayArray30 entry_name", asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH, entry_name[0][0]));
   ScriptEngine->RegisterObjectProperty("Strength", "const CharArray unkwn1",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH, unkwn1[0]));
 
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 dvx",        asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, dvx));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 dvy",        asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, dvy));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 fall",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, fall));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 arest",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, arest));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 vrest",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, vrest));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 respond",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, respond));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 effect",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, effect));
   ScriptEngine->RegisterObjectProperty("Entry", "const CharArray unkwn1", asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, unkwn1[0]));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 bdefend",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, bdefend));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 injury",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, injury));
   ScriptEngine->RegisterObjectProperty("Entry", "const int32 zwidth",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, zwidth));
   ScriptEngine->RegisterObjectProperty("Entry", "const CharArray unkwn2", asOFFSET(AISCRIPT_NGAME_FILE_DATA_STRENGTH_ENTRY, unkwn2[0]));

   ScriptEngine->RegisterObjectProperty("Frame", "const bool  exists",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, exists));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 pic",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, pic));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 state",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, state));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 wait",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, wait));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 next",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, next));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 dvx",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, dvx));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 dvy",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, dvy));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 dvz",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, dvz));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn1",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn1));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_a",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_a));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_d",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_d));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_j",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_j));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_Fa",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_Fa));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_Ua",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_Ua));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_Da",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_Da));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_Fj",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_Fj));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_Uj",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_Uj));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_Dj",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_Dj));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 hit_ja",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, hit_ja));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 mp",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, mp));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 centerx", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, centerx));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 centery", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, centery));
   ScriptEngine->RegisterObjectProperty("Frame", "const Opoint opoint", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, opoint));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn2",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn2));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn3",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn3));
   ScriptEngine->RegisterObjectProperty("Frame", "const Bpoint bpoint", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, bpoint));
   ScriptEngine->RegisterObjectProperty("Frame", "const Cpoint cpoint", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, cpoint));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn4",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn4));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn5",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn5));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn6",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn6));
   ScriptEngine->RegisterObjectProperty("Frame", "const Wpoint wpoint", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, wpoint));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn7",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn7));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn8",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn8));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn9",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn9));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn10", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn10));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn11", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn11));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn12", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn12));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn13", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn13));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn14", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn14));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn15", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn15));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn16", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn16));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn17", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn17));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 itr_count", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, itr_count));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 bdy_count", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, bdy_count));
   ScriptEngine->RegisterObjectProperty("Frame", "const ItrArray @itrs",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, itrs));
   ScriptEngine->RegisterObjectProperty("Frame", "const BdyArray @bdys",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, bdys));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 itr_x",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, itr_x));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 itr_y",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, itr_y));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 itr_w",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, itr_w));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 itr_h",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, itr_h));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 bdy_x",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, bdy_x));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 bdy_y",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, bdy_y));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 bdy_w",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, bdy_w));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 bdy_h",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, bdy_h));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn18", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn18));
   ScriptEngine->RegisterObjectProperty("Frame", "const CharArray fname",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, fname));
   ScriptEngine->RegisterObjectProperty("Frame", "const CharArray @sound", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, sound));
   ScriptEngine->RegisterObjectProperty("Frame", "const int32 unkwn19",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME, unkwn19));

   ScriptEngine->RegisterObjectProperty("Bpoint", "const int32 x", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BPOINT, x));
   ScriptEngine->RegisterObjectProperty("Bpoint", "const int32 y", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BPOINT, y));

   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 kind",         asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, kind));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 x",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, x));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 y",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, y));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 injury",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, injury));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 cover",        asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, cover));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 fronthurtact", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, injury));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 backhurtact",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, cover)); 
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 vaction",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, vaction));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 aaction",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, aaction));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 jaction",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, jaction));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 daction",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, daction));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 throwvx",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, throwvx));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 throwvy",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, throwvy));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 hurtable",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, hurtable));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 decrease",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, decrease));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 dircontrol",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, dircontrol));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 taction",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, taction));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 throwinjury",  asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, throwinjury));
   ScriptEngine->RegisterObjectProperty("Cpoint", "const int32 throwvz",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_CPOINT, throwvz));

   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 kind",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, kind));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 x",         asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, x));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 y",         asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, y));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 weaponact", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, weaponact));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 attacking", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, attacking));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 cover",     asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, cover));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 dvx",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, dvx));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 dvy",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, dvy));
   ScriptEngine->RegisterObjectProperty("Wpoint", "const int32 dvz",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_WPOINT, dvz));

   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 kind",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, kind));
   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 x",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, x));
   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 y",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, y));
   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 action", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, action));
   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 dvx",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, dvx));
   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 dvy",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, dvy));
   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 oid",    asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, oid));
   ScriptEngine->RegisterObjectProperty("Opoint", "const int32 facing", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_OPOINT, facing));

   ScriptEngine->RegisterObjectProperty("Itr", "const int32 kind",         asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, kind));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 x",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, x));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 y",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, y));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 w",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, w));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 h",            asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, h));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 dvx",          asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, dvx));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 dvy",          asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, dvy));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 fall",         asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, fall));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 arest",        asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, arest));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 vrest",        asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, vrest));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 respond",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, respond));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 effect",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, effect));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 catchingact1", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, catchingact1));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 catchingact2", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, catchingact2));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 caughtact1",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, caughtact1));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 caughtact2",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, caughtact2));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 bdefend",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, bdefend));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 injury",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, injury));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 zwidth",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, zwidth));
   ScriptEngine->RegisterObjectProperty("Itr", "const int32 unkwn2",       asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_ITR, unkwn1));

   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 kind",   asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, kind));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 x",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, x));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 y",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, y));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 w",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, w));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 h",      asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, h));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 unkwn1", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, unkwn1));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 unkwn2", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, unkwn2));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 unkwn3", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, unkwn3));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 unkwn4", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, unkwn4));
   ScriptEngine->RegisterObjectProperty("Bdy", "const int32 unkwn5", asOFFSET(AISCRIPT_NGAME_FILE_DATA_FRAME_BDY, unkwn5));

   ScriptEngine->RegisterObjectProperty("Stage", "const int32 phase_count", asOFFSET(AISCRIPT_NGAME_FILE_STAGE, phase_count));
   ScriptEngine->RegisterObjectProperty("Stage", "const PhaseArray phases", asOFFSET(AISCRIPT_NGAME_FILE_STAGE, phases));

   ScriptEngine->RegisterObjectProperty("Phase", "const int32 bound",       asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE, bound));
   ScriptEngine->RegisterObjectProperty("Phase", "const CharArray  music",  asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE, music));
   ScriptEngine->RegisterObjectProperty("Phase", "const SpawnArray spawns", asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE, spawns));
   ScriptEngine->RegisterObjectProperty("Phase", "const int32 when_clear_goto_phase", asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE, when_clear_goto_phase));
 
   ScriptEngine->RegisterObjectProperty("Spawn", "const IntArray unkwn1",     asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, unkwn1));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  id",           asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, id));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  x",            asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, x));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  hp",           asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, hp));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  times",        asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, times));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  reserve",      asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, reserve));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  join",         asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, join));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  join_reserve", asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, join_reserve));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  act",          asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, act));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  y",            asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, y));
   ScriptEngine->RegisterObjectProperty("Spawn", "const double ratio",        asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, ratio));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  role",         asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, role));
   ScriptEngine->RegisterObjectProperty("Spawn", "const int32  unkwn2",       asOFFSET(AISCRIPT_NGAME_FILE_STAGE_PHASE_SPAWN, unkwn2));
  
   ScriptEngine->RegisterObjectProperty("Background", "const int32 bg_width",     asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, bg_width));
   ScriptEngine->RegisterObjectProperty("Background", "const int32 bg_zwidth1",   asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, bg_zwidth1));
   ScriptEngine->RegisterObjectProperty("Background", "const int32 bg_zwidth2",   asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, bg_zwidth2));
   ScriptEngine->RegisterObjectProperty("Background", "const int32 perspective1", asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, perspective1));
   ScriptEngine->RegisterObjectProperty("Background", "const int32 perspective2", asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, perspective2));
   ScriptEngine->RegisterObjectProperty("Background", "const int32 shadow1",      asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, shadow1));
   ScriptEngine->RegisterObjectProperty("Background", "const int32 shadow2",      asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, shadow2));
   ScriptEngine->RegisterObjectProperty("Background", "const int32 layer_count",  asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, layer_count));
   ScriptEngine->RegisterObjectProperty("Background", "const CharArrayArray30 layer_bmps", asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, layer_bmps));
   ScriptEngine->RegisterObjectProperty("Background", "const CharArray shadow_bmp", asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, shadow_bmp));
   //ScriptEngine->RegisterObjectProperty("Background", "const CharArray unkwn1",     asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, unkwn1));
   ScriptEngine->RegisterObjectProperty("Background", "const CharArray name",       asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, name));
   //ScriptEngine->RegisterObjectProperty("Background", "const CharArray unkwn2",     asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, unkwn2));
   ScriptEngine->RegisterObjectProperty("Background", "const LayerArray layer",     asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, layer));
   ScriptEngine->RegisterObjectProperty("Background", "const IntArray layer_ptrs",  asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, layer_ptrs));
   ScriptEngine->RegisterObjectProperty("Background", "const int ptr",              asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND, ptrs));

   ScriptEngine->RegisterObjectProperty("Layer", "const int32 transparency", asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, transparency));
   ScriptEngine->RegisterObjectProperty("Layer", "const int32 width",        asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, width));
   ScriptEngine->RegisterObjectProperty("Layer", "const int32 x",            asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, x));
   ScriptEngine->RegisterObjectProperty("Layer", "const int32 y",            asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, y));
   ScriptEngine->RegisterObjectProperty("Layer", "const int32 height",       asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, height));
   //ScriptEngine->RegisterObjectProperty("Layer", "const int32 loop",         asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, loop));
   //ScriptEngine->RegisterObjectProperty("Layer", "const int32 c1",           asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, c1));
   //ScriptEngine->RegisterObjectProperty("Layer", "const int32 c2",           asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, c2));
   //ScriptEngine->RegisterObjectProperty("Layer", "const int32 cc",           asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, cc));
   //ScriptEngine->RegisterObjectProperty("Layer", "const int32 animation",    asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, animation));
   //ScriptEngine->RegisterObjectProperty("Layer", "const int32 rect",         asOFFSET(AISCRIPT_NGAME_FILE_BACKGROUND_LAYER, rect));
  } else
  {
  ...
  if(neora_mode)
  {
   ScriptEngine->RegisterObjectMethod("ObjectArray",      "const Object &opIndex(uint x) const",     asMETHOD(AISCRIPT_ARRAY_OBJECT, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("DataFileArray",    "const DataFile &opIndex(uint x) const",   asMETHOD(AISCRIPT_ARRAY_DATA, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("FileArray",        "const File &opIndex(uint x) const",       asMETHOD(AISCRIPT_ARRAY_FILE, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("EntryArray",       "const Entry &opIndex(uint x) const",      asMETHOD(AISCRIPT_ARRAY_ENTRY, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("FrameArray",       "const Frame &opIndex(uint x) const",      asMETHOD(AISCRIPT_ARRAY_FRAME, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("ItrArray",         "const Itr &opIndex(uint x) const",        asMETHOD(AISCRIPT_ARRAY_ITR, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("BdyArray",         "const Bdy &opIndex(uint x) const",        asMETHOD(AISCRIPT_ARRAY_BDY, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("StageArray",       "const Stage &opIndex(uint x) const",      asMETHOD(AISCRIPT_ARRAY_STAGE, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("PhaseArray",       "const Phase &opIndex(uint x) const",      asMETHOD(AISCRIPT_ARRAY_PHASE, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("SpawnArray",       "const Spawn &opIndex(uint x) const",      asMETHOD(AISCRIPT_ARRAY_SPAWN, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("BackgroundArray",  "const Background &opIndex(uint x) const", asMETHOD(AISCRIPT_ARRAY_BACKGROUND, optIndex), asCALL_THISCALL);
   ScriptEngine->RegisterObjectMethod("LayerArray",       "const Layer &opIndex(uint x) const",      asMETHOD(AISCRIPT_ARRAY_LAYER, optIndex), asCALL_THISCALL);
  } else
  {
  ...
  if(neora_mode)
  {ScriptEngine->RegisterObjectProperty("Info", "DataFile @data",     asOFFSET(AISCRIPT_INFO, Ndata));} else
  ...
  if(neora_mode)
  {
   //ScriptEngine->RegisterGlobalProperty("const Game  game", Ngame);
   #if _AISCRIPT_NEORACOMPATIBLE
    ScriptEngine->RegisterObjectProperty("Game2", "Game game",  asOFFSET(AISCRIPT_NGAMEPTR, game));
    ScriptEngine->RegisterGlobalProperty("const Game2 gamePtr", neo);
   #endif
  } else
  {
 ...
 int0 M_Starting() perfect
 {
  ...
  #if _AISCRIPT_NEORACOMPATIBLE
   neora_mode = _AISCRIPT_FORPDKNNEORAEXE;
   if(!neora_mode) neora_mode = L_Exist("neora.dll");
  #endif
